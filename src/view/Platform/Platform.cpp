...
namespace Game_Repr {
Platform::Platform()
    : mPlatform(std::make_shared<sf::RectangleShape>(sf::Vector2f(Config::platformWidth, Config::platformHeight))) {
    mPlatform->setOutlineThickness(2.f);
    mPlatform->setOutlineColor(sf::Color::Black);
}

const std::shared_ptr<sf::RectangleShape>& Platform::getMPlatform() const { return mPlatform; }

void Platform::setMPlatform(const std::shared_ptr<sf::RectangleShape>& n_mPlatform) {
    Platform::mPlatform = n_mPlatform;
}

const Coordinates& Platform::getPosition() const { return position; }

void Platform::setPosition(const Coordinates& n_position) { Platform::position = n_position; }
...
Platform::~Platform() { mPlatform = nullptr; }

const std::shared_ptr<Game_Repr::Bonus>& Platform::getBonus() const { return bonus; }

void Platform::setBonus(const std::shared_ptr<Bonus>& n_bonus) { Platform::bonus = n_bonus; }
