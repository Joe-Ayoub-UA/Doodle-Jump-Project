...
Game_Repr::Player::Player()
    : mPlayer(std::make_shared<sf::Sprite>()), mHitboxDebug(std::make_shared<sf::RectangleShape>()) {
...
const std::shared_ptr<sf::Sprite>& Game_Repr::Player::getMPlayer() const { return mPlayer; }

void Game_Repr::Player::setMPlayer(const std::shared_ptr<sf::Sprite>& n_mPlayer) { Player::mPlayer = n_mPlayer; }

Player::~Player() { mPlayer = nullptr; }

const Coordinates& Player::getPosition() const { return position; }

void Player::setPosition(const Coordinates& n_position) { Player::position = n_position; }
...
std::pair<float, float> Player::getMDimensions() const { return std::make_pair(mDimensions.x, mDimensions.y); }

const std::shared_ptr<sf::RectangleShape>& Player::getMHitboxDebug() const { return mHitboxDebug; }
} // namespace Game_Repr