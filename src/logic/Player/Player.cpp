...
Coordinates Player::teleportPlayer(const Coordinates& coordinate) {
    Coordinates coordinates =
        Coordinates(observer->getMPlayer()->getPosition().x, observer->getMPlayer()->getPosition().y);
...
void Player::assignObserver(std::shared_ptr<Game_Repr::Player> newObserver) { this->observer = std::move(newObserver); }

void Player::deleteObserver() { observer = nullptr; }

void Player::notifyObserver() { observer->update(); }

void Player::notifyPosition(const Coordinates& coordinates) { observer->notifyPosition(coordinates); }

std::shared_ptr<Game_Repr::Player> Player::getObserver() { return this->observer; }

float Player::getVerticalSpeed() const { return verticalSpeed; }

void Player::setVerticalSpeed(float n_verticalSpeed) { Player::verticalSpeed = n_verticalSpeed; }

float Player::getHorizontalSpeed() const { return horizontalSpeed; }

void Player::setHorizontalSpeed(float n_horizontalSpeed) { Player::horizontalSpeed = n_horizontalSpeed; }

float Player::getJumpForce() const { return jumpForce; }
void Player::setJumpForce(float n_jumpForce) { Player::jumpForce = n_jumpForce; }
} // namespace Logic_Library